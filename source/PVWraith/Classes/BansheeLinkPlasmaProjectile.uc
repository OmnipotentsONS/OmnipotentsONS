...
class BansheeLinkPlasmaProjectile extends ONSPlasmaProjectile;
...
#exec audio import file=Sounds\BansheeLinkPlasmaHit.wav
#exec audio import file=Sounds\BansheeLinkPlasmaBounce.wav
...
simulated function ProcessTouch(Actor Other, vector HitLocation)
{
	...
	local BansheeLinkPlasmaProjectile P;
	...
	if (xPawn(Other) != None && xPawn(Other).CheckReflect(HitLocation, RefNormal, Damage * 0.25))
	{
		...
		Other.PlaySound(Sound'BansheeLinkPlasmaBounce',, TransientSoundVolume,, TransientSoundRadius);
		Other.MakeNoise(0.3);
		Destroy();
	}
	else
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	PlaySound(Sound'BansheeLinkPlasmaHit');
...
simulated function Bounce(vector HitLocation, vector HitNormal)
{
	...
	PlaySound(Sound'BansheeLinkPlasmaBounce');
...
defaultproperties
{
     ...
     MyDamageType=Class'PVWraith.DamTypeBansheeLinkPlasma'
     LifeSpan=2.000000
     bBounce=True
}
