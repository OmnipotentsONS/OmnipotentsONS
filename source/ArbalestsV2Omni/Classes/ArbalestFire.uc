class ArbalestFire extends Projectile;
...
simulated function PostBeginPlay()
{
    ...
    NapalmFireEffect = Spawn(class'FireVehiclesV2Omni.FireTankFireNew',self);
...
auto state Flying
{
    ...
    {
        local FireTankSmallBlob Glob;

        Glob = FireTankSmallBlob(Other);
...
state OnGround
{
    ...
    if ( Role == ROLE_Authority )
	{
		for (i=0; i<3; i++)
		{
			...
			NTF = Spawn( class 'ArbalestSmallBlob',, '', Start, rot);
		}
	}
    ...
    {
        if (DamageType.default.bDetonatesGoop  && DamageType.class != class'FireVehiclesV2Omni.FireKill')
        {
            bDrip = false;
            SetTimer(0.1, false);
        }
...
defaultproperties
{
     ...
     MyDamageType=Class'FireVehiclesV2Omni.FireKill'
