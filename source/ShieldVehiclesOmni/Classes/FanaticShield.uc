...
//
//-----------------------------------------------------------
class FanaticShield extends Actor;
...
{
	if (FanaticShieldCannon(Owner) != None)
        FanaticShieldCannon(Owner).NotifyShieldHit(Dam, instigatedBy);
}
...
simulated function SpawnHitEffect(byte TeamNum)
{
    if (Level.NetMode != NM_DedicatedServer)
    {
        ...
        {
            if (TeamNum == 1)
                ShockShieldHitEffect = spawn(class'FanaticShieldHitEffectBlue', self);
            else
                ShockShieldHitEffect = spawn(class'FanaticShieldHitEffectRed', self);
        }

        if (ShockShieldHitEffect != None && Owner != None && FanaticShieldCannon(Owner) != None)
...
simulated function ActivateShield(byte TeamNum)
{
    ...
    if (Level.NetMode != NM_DedicatedServer)
    {
        ...
        {
            if (TeamNum == 1)
                ShockShieldEffect = spawn(class'FanaticShieldEffectBlue', self);
            else
                ShockShieldEffect = spawn(class'FanaticShieldEffectRed', self);

            PlaySound(sound'ONSBPSounds.ShockTank.ShieldActivate', SLOT_None, 2.0);
        }

        if (ShockShieldEffect != None && Owner != None && FanaticShieldCannon(Owner) != None)
            Owner.AttachToBone(ShockShieldEffect, 'ElectroGun');
    }
}
