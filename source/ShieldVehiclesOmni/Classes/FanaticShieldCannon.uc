...
//
//-----------------------------------------------------------
class FanaticShieldCannon extends ONSWeapon;
...
simulated function PostNetBeginPlay()
{
    ...
    ShockShield = spawn(class'FanaticShield', self);
...
simulated function Tick(float DT)
{
    Super.Tick(DT);
 	...
 	if (Fanatic(Owner) != None) 
		  { MyFanatic=Fanatic(Owner);
		    NumLinks = MyFanatic.Links;
		  }
		else  NumLinks = 0;
    ...
    // only recharge while up if Fanatic is being linkend
    {
...
//  log(self@":HurtRadius - Start");
	...
	MyFanatic.GiveHealth(SelfHealAmount, MyFanatic.HealthMax);
  //log(self@":HurtRadius - AFTER SelfHealBaseAmount-CurrentHealth"@ONSWeaponPawn(Owner).VehicleBase.Health);
	...
	MyFanatic.GiveHealth(FMin(damageTotal * SelfHealMultiplier, MaxDamageHealthHeal), MyFanatic.HealthMax);
...
defaultproperties
{
    ...
    // PitchBone="SIDEgunBARREL"
     ...
     BlueSkin=Texture'IllyCrusaderSkins.Crusader.Crusader_1'
     ...
     ProjectileClass=Class'ShieldVehiclesOmni.FanaticShieldCannonProjectile'
