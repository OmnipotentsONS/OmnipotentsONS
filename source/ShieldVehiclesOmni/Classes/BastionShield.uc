...
//
//-----------------------------------------------------------
class BastionShield extends Actor;
...
{
	if (BastionShieldCannon(Owner) != None)
        BastionShieldCannon(Owner).NotifyShieldHit(Dam, instigatedBy);
}
...
simulated function SpawnHitEffect(byte TeamNum)
{
    if (Level.NetMode != NM_DedicatedServer)
    {
        ...
        {
            if (TeamNum == 1)
                ShockShieldHitEffect = spawn(class'BastionShieldHitEffectBlue', self);
            else
                ShockShieldHitEffect = spawn(class'BastionShieldHitEffectRed', self);
        }

        if (ShockShieldHitEffect != None && Owner != None && BastionShieldCannon(Owner) != None)
...
simulated function ActivateShield(byte TeamNum)
{
    ...
    if (Level.NetMode != NM_DedicatedServer)
    {
        ...
        {
           if (TeamNum == 1)
                ShockShieldEffect = spawn(class'BastionShieldEffectBlue', self);
            else
                ShockShieldEffect = spawn(class'BastionShieldEffectRed', self);
            PlaySound(sound'ONSBPSounds.ShockTank.ShieldActivate', SLOT_None, 2.0);
        }

        if (ShockShieldEffect != None && Owner != None && BastionShieldCannon(Owner) != None)
            Owner.AttachToBone(ShockShieldEffect, 'ElectroGun');
    }
}
