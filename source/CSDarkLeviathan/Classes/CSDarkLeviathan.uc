...
class CSDarkLeviathan extends ONSWheeledCraft;

#exec OBJ LOAD FILE=..\Sounds\MenuSounds.uax
#exec OBJ LOAD FILE=..\Textures\ONSFullTextures.utx
#exec OBJ LOAD FILE="Textures\CSDarkLeviathan_Tex.utx" package=CSDarkLeviathan
#exec AUDIO IMPORT FILE="Sounds\ImATerminator.wav"
#exec AUDIO IMPORT FILE="Sounds\ithinkican.wav"
...
{
    super.PostBeginPlay();
    SetBoneLocation('RightRearGunAttach', (vect(0,0,6)));
	SetBoneLocation('LeftRearGunAttach', (vect(0,0,6)));
}
...
Begin:
    if (Controller != None)
    {
        ...
        Weapons[1].PlayAnim('MASMainGunHide', 3.0);
        sleep(2.3/3);
        PlayAnim('MASMainGunHide',3.0);
        sleep(4.03/3);
        bMovable = True;
...
Begin:
    if (Controller != None)
    {
        ...
        PlayAnim('MASMainGunDeploy',3.0);
        sleep(3.46/3);
        Weapons[1].PlayAnim('MASMainGunDeploy',3.0);
        sleep(2.873/3);
...
function ServerPlayHorn(int HornIndex)
{
	...
	{
        if(HornIndex != 1)
            PlaySound( HornSounds[HornIndex],, 6.0*TransientSoundVolume,, 1200);
        else
...
DefaultProperties
{
    ...
    ServerPhysics=PHYS_Karma
    bNetNotify=True
	VehiclePositionString="in a Dark Leviathan"
	VehicleNameString="Dark Leviathan 1.1"
	...
	PassengerWeapons(0)=(WeaponPawnClass=class'CSDarkLeviRailTurretPawn',WeaponBone=RightFrontGunAttach);
	PassengerWeapons(1)=(WeaponPawnClass=class'CSDarkLeviRailTurretPawn',WeaponBone=LeftFrontGunAttach);
	PassengerWeapons(2)=(WeaponPawnClass=class'CSDarkLeviFlakTurretPawn',WeaponBone=RightRearGunAttach);
	PassengerWeapons(3)=(WeaponPawnClass=class'CSDarkLeviFlakTurretPawn',WeaponBone=LeftRearGunAttach);
    ...
    RedSkin=Texture'CSDarkLeviathan.CSDarkLeviathan_Red'
    BlueSkin=Texture'CSDarkLeviathan.CSDarkLeviathan_Blue'
	...
	TorqueCurve=(Points=((InVal=0,OutVal=500.0),(InVal=5000,OutVal=1.5),(InVal=25000000,OutVal=0.0)))
...
//		SupportBoneName="LeftFrontStrut"
	...
	HornSounds(0)=sound'CSDarkLeviathan.ithinkican'
	HornSounds(1)=sound'ONSVehicleSounds-S.LevHorn02'
	HornSounds(2)=sound'CSDarkLeviathan.ImATerminator'
