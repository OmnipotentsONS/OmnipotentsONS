class CSPallasTurretLaserCannon extends ONSWeapon;
...
state InstantFireMode
{
    function TraceFire(Vector Start, Rotator Dir)
    {
        ...
        //if we don't do this override then projectile hits the vehicle
	    if ( WeaponPawn != None && WeaponPawn.VehicleBase != None )
    	{
    		if ( !WeaponPawn.VehicleBase.TraceThisActor(HitLocation, HitNormal, Start, Start + Vector(Dir) * (WeaponPawn.VehicleBase.CollisionRadius * 1.0)))
            {
				Start = HitLocation;
            }
		}
...
defaultproperties
{
     ...
     FireForce="Laser01"
     DamageType=Class'CSPallasV2.CSPallasDamTypeLaserCannon'
     DamageMin=9
     DamageMax=9
