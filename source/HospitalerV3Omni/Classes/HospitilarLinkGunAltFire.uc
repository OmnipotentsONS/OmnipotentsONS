class HospitilarLinkGunAltFire extends LinkTAltFire;

function PlayFiring()
{
	if ( LinkGun(Weapon).Links <= 0 )
		ClientPlayForceFeedback("BLinkGunBeam1");
	HospitilarLinkGunPawn(Instigator).bIsShooting= true;
...
function StopFiring()
{
	...
	HospitilarLinkGunPawn(Instigator).bIsShooting = false;
    if (Beam != None)
    {
        Beam.Destroy();
        Beam = None;
...
simulated function ModeTick(float dt)
{
	...
	// try not to link to the base vehicle
      	ONSWeaponPawn(Instigator).GetVehicleBase().bBlockZeroExtentTraces = False;
        Other = Weapon.Trace(HitLocation, HitNormal, EndTrace, StartTrace, true);
      	ONSWeaponPawn(Instigator).GetVehicleBase().bBlockZeroExtentTraces = True;
        ...
        if ( Other != None && Other != Instigator )
        {
            ...
            if ( IsLinkable(Other) && (LinkedVehicle != ONSWeaponPawn(Instigator).GetVehicleBase()))
            {
                if ( Other != lockedpawn )
		...
		if (lockedpawn==ONSWeaponPawn(Instigator).GetVehicleBase())
		{
			SetLinkTo( None );
		}
                // stop linking
		...
		if ( LinkedVehicle != None && bDoHit && (LinkedVehicle != ONSWeaponPawn(Instigator).GetVehicleBase()))
		{
...
defaultproperties
{
     BeamEffectClass=Class'HospitalerV3Omni.HospitilarLinkGunBeamEffect'
}
