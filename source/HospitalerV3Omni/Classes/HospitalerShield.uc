...
//
//-----------------------------------------------------------
class HospitalerShield extends Actor;
...
{
	if (HospitalerShieldCannon(Owner) != None)
        HospitalerShieldCannon(Owner).NotifyShieldHit(Dam, instigatedBy);
}
...
simulated function SpawnHitEffect(byte TeamNum)
{
    if (Level.NetMode != NM_DedicatedServer)
    {
        ...
        {
            if (TeamNum == 1)
                ShockShieldHitEffect = spawn(class'HospitalerShieldHitEffectBlue', self);
            else
                ShockShieldHitEffect = spawn(class'HospitalerShieldHitEffectRed', self);
        }

        if (ShockShieldHitEffect != None && Owner != None && HospitalerShieldCannon(Owner) != None)
...
simulated function ActivateShield(byte TeamNum)
{
    ...
    if (Level.NetMode != NM_DedicatedServer)
    {
        ...
        if (ShockShieldEffect != None && Owner != None && HospitalerShieldCannon(Owner) != None)
            Owner.AttachToBone(ShockShieldEffect, 'SIDEgunBARREL');
    }
}
