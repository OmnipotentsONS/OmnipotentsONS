...
Class ONSPlusPlayerReplicationInfo extends ONSPlayerReplicationInfo
...
// Code for word/name filtering
var array<ONSPlusMutator.EFilteredWord> WordFilterList;
var byte FailedWordSends;
var bool bInitialisedWordList;
var bool bFilterSentWords;
var bool bFilterAdmins;
var int FilterLength;
var int MaxWordHits;
var ONSPlusMutator.EMaxHitAction MaxHitAction;
var int WordHits;

var array<String> WordList;
var int WListCount;
var FileLog WordData;
var PlayerController WordSender;

// Stuff for the action plan list (note: I need to fix this so it can't be hacked to peek at the other teams plans, fuck ut2k4's bad/inflexable variable handling)
//var int AssignedNode;
...
replication
{
	...
	reliable if (Role == ROLE_Authority)
		ClientUpdateFactoryList, ClientUpdateFactoryListTeam, ClientUpdateFactoryClass/*, AssignedNode*/;//, bFilterSentWords;

	reliable if (Role == ROLE_Authority && bNetOwner)
		bFilterSentWords, bFilterAdmins;

	reliable if (Role < ROLE_Authority)
		ONSPlusTeleportTo, ONSPlusSetStartCore, RequestVehicleInfoUpdate, HitWord;
		//ONSPlusTeleportTo, ONSPlusSetStartCore, RequestVehicleInfoUpdate, HitWord, ServerResendFilter;//, RequestWordFilterResend;
}

function UpdateWordFilter(ONSPlusMutator.EFilteredWord NewWord, int Position)
{
	if (NewWord.Word ~= "")
		return;

	// Don't bother to scan the list to make sure a word isn't already in it, waste of processing power on server..just add to own list & directly send
	// N.B. REMOVED NEED TO STORE WORDS SERVERSIDE (waste of memory)

	// TODO: Add a 'position' number to this function so you can access the filter list in mutator with the same number
	//WordFilterList.Length = WordFilterList.Length + 1;
	//WordFilterList[WordFilterList.Length - 1] = NewWord;

	//ClientUpdateWordFilter(NewWord, FilterLength);
	ClientUpdateWordFilter(NewWord, Position);

	//FilterLength++;
}

simulated function ClientUpdateWordFilter(ONSPlusMutator.EFilteredWord NewWord, int Position)
{
	// Update list
	WordFilterList.Insert(Position, 1);
	WordFilterList[Position] = NewWord;
}

//function HitWord(int HitCount)
function HitWord(int ListIndex)
{
	//if (HitCount < 1)
	if (MutatorOwner == none || bAdmin || MutatorOwner.BlacklistedWords[ListIndex].HitCount < 1)
		return;

	//WordHits += HitCount;

	// This function is serverside, meaning bad words are stored in the MUTATOR not copied into every PRI (only the client stores them in the PRI)
	WordHits += MutatorOwner.BlacklistedWords[ListIndex].HitCount;

	if (WordHits > MaxWordHits)
	{
		if (MaxHitAction == HA_Kick)
		{
			Level.Game.AccessControl.DefaultKickReason = "Kicked by swear filter";
			Level.Game.AccessControl.KickPlayer(PlayerController(Owner));
			Level.Game.AccessControl.DefaultKickReason = Level.Game.AccessControl.default.DefaultKickReason;
		}
		else if (MaxHitAction == HA_SessionBan)
		{
			Level.Game.AccessControl.BanPlayer(PlayerController(Owner), true);
		}
		else
		{
			Level.Game.AccessControl.BanPlayer(PlayerController(Owner));
		}
	}
}

/*function RequestWordFilterResend(int Position)
{
	if (FailedWordSends > 10 || Position > WordFilterList.Length - 1)
		return;

	FailedWordSends++;

	ClientUpdateWordFilter(WordFilterList[Position], Position);
}*/
