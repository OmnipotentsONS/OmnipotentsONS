...
class KBMercuryMissile extends Projectile;
...
var KBMercuryMissileTrail Trail;
...
simulated function PostBeginPlay()
{
	...
	if (Level.NetMode != NM_DedicatedServer)
		Trail = Spawn(class'KBMercuryMissileTrail', self,, Location, Rotation);
...
simulated function ProcessContact(bool bPenetrate, Actor Other, vector HitLocation, vector HitNormal)
{
	...
	if (PC != None && (HeadshotPawn == None || HeadshotPawn.bDeleteMe || HeadshotPawn.Health <= 0)) {
		PC.ReceiveLocalizedMessage(class'KBHeadshotVictimMessage',, InstigatorController.PlayerReplicationInfo);
	}
}
...
{
	local PlayerController PC;
	local class<KBMercuryExplosion> ExplosionClass;
...
*/
simulated function class<KBMercuryExplosion> GetExplosionClass(Actor HitActor, vector HitLocation, vector HitNormal, out rotator EffectRotationOffset)
{
	...
	if (PhysicsVolume.bWaterVolume) {
		return class'KBMercuryExplosion';
	}
	...
	if (HitMaterial != None) {
		switch (HitMaterial.SurfaceType) {
			...
			case EST_Wood:
			case EST_Plant:
				return class'KBMercuryExplosionDirt';
			case EST_Ice:
			case EST_Snow:
				return class'KBMercuryExplosionSnow';
		}
	}
...
defaultproperties
{
     SplashDamageType=Class'KopholusV2Omni.KBDamTypeMercurySplashDamage'
     SplashMomentum=10000.000000
     TransferDamageAmount=0.003000
     ImpactDamageAmount=50.000000
     HeadHitDamage=Class'KopholusV2Omni.KBDamTypeMercuryHeadHit'
     DirectHitDamage=Class'KopholusV2Omni.KBDamTypeMercuryDirectHit'
     PunchThroughDamage=Class'KopholusV2Omni.KBDamTypeMercuryPunchThrough'
     ThroughHeadDamage=Class'KopholusV2Omni.KBDamTypeMercuryPunchThroughHead'
     ...
     TeamSkins(3)=TexScaler'WVMercuryMissiles_Tex.Skins.MercuryMissileTexGold'
     ...
     MyDamageType=Class'KopholusV2Omni.KBDamTypeMercuryDirectHit'
     ImpactSound=Sound'WVMercuryMissilesSounds.Effects.MercPunchThrough'
     ExplosionDecal=Class'KopholusV2Omni.KBMercuryImpactMark'
     LightType=LT_Steady
