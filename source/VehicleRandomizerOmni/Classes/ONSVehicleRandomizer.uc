...
//=============================================================================
class ONSVehicleRandomizer extends Info
	placeable;
// Updates for Omni )o( by pOOty
// Support for ignorning RandomVehicleFactories, in addtion to ForcedFactories
...
struct ArenaVehicleDefinition
{
	...
	var() bool bDisableForcedVehicles;		// If true, ForcedVehicle factories are disabled for this round
	var() bool bReplaceForcedVehicles;		// If true, ForcedVehicle factories spawn the arena vehicle
};
...
//				log("First node of group -- Create vehicle list.",'KDebug');
				...
				while ((CurrentWorth < NodeGroups[i].MinWorth || CurrentWorth > NodeGroups[i].MaxWorth) && (Iter < 100))
				{
					...
					for (k = 0; k < UseFactories.Length; k++)
					{
						...
						if (!UseFactories[k].isA('ONSForcedVehicleFactory') 
							&& !UseFactories[k].IsA('ONSTurretFactory') )
						{
...
//					log("Final vehicle setup is worth"@CurrentWorth@"-- Target worth between"@NodeGroups[i].MinWorth@"and"@NodeGroups[i].MaxWorth,'XADebug');
			...
			for (k = 0; k < UseFactories.Length; k++)
			{
				if (ONSForcedVehicleFactory(UseFactories[k]) == None
					&& !UseFactories[k].IsA('ONSTurretFactory'))
				{
