//-----------------------------------------------------------
// StingRay - Manta variant by ~Ca5ti3l~ (c) 2016.
// I have never seen a Manta with passenger capabilities, so I decided to made one.
//-----------------------------------------------------------
class StingRay extends ONSHoverCraft;
...
//exec OBJ LOAD FILE=..\CastielVehicle Pack1\textures\Cas-StingRay.utx
...
simulated function Tick(float DeltaTime)
{
	...
	if(DoBikeJump != OldDoBikeJump)
	{
        ...
        {
            JumpEffect = Spawn(class'OmniMantas.StingRayJumpEffect');
...
simulated function CheckJumpDuck()
{
    ...
    else if (DuckCountdown <= 0.0 && (Rise < 0 || bWeaponIsAltFiring))
    {
        if (!bHoldingDuck)
        {
			...
			if(Level.NetMode != NM_DedicatedServer)
			{
				DuckEffect = Spawn(class'OmniMantas.StingRayDuckEffect');
...
defaultproperties
{
     ...
     DriverWeapons(0)=(WeaponClass=Class'OmniMantas.StingRayGun',WeaponBone="PlasmaGunAttachment")
     PassengerWeapons(0)=(WeaponPawnClass=Class'OmniMantas.StingRayTurretPawn',WeaponBone="PlasmaGunAttachment")
     bHasAltFire=False
     RedSkin=Texture'Cas-StingRay.StingRayTextures.StingRayRed1'
     BlueSkin=Texture'Cas-StingRay.StingRayTextures.StingRayBlue1'
     ...
     VehiclePositionString="in the Sting Ray"
     VehicleNameString="Sting Ray 1.01"
     ...
     End Object
     KParams=KarmaParamsRBFull'OmniMantas.Stingray.KParams0'
