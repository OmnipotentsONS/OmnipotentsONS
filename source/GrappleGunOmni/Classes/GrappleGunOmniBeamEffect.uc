class GrappleGunOmniBeamEffect extends xEmitter
	notplaceable;
...
var GrappleGunOmniBeamSparks			Sparks;
var GrappleGunOmniMuzFlash MuzFlash, OldMuzFlash;
...
simulated function Tick(float dt)
{
	...
	if ( Level.NetMode != NM_DedicatedServer )
	{
		...
		{
			if ( MuzFlash == None )
				MuzFlash = Spawn(class'GrappleGunOmniMuzFlash', self);
		}
		...
		if ( Sparks == None && EffectIsRelevant(EndEffect, false) )
		{
			...
			if ( (P == Instigator.Controller) || CheckMaxEffectDistance(P, Location) )
				Sparks = Spawn(class'GrappleGunOmniBeamSparks', self);
		}
	}
    ls = class'GrappleGunOmniFire'.default.LinkScale[Min(Links,5)];
   ...
   if ( LinkColor == 0 )
        ...
        {
            Skins[0] = FinalBlend'GrappleGunOmni_Tex.Weapon.BeamFB';
        }
        else
        {
            Skins[0] = FinalBlend'GrappleGunOmni_Tex.Weapon.BeamFB';
