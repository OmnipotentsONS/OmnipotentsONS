...
// ============================================================================
class ONSLinkTankTertiaryTurret extends ONSWeapon;

var() sound LinkedFireSound;

var class<ONSTurretBeamEffect> BeamEffectClass[2];
var float LinkDamageMult;
...
simulated event OwnerEffects()
{
	...
	{
		if (ONSLinkTank(ONSWeaponPawn(Owner).VehicleBase) != None)
			NumLinks = ONSLinkTank(ONSWeaponPawn(Owner).VehicleBase).GetLinks();
		else
			NumLinks = 0;
	...
	if (!Other.bWorldGeometry)
        {
            ...
            Other.TakeDamage(Damage * (1.0 + float(NumLinks)), Instigator, HitLocation, Momentum*X, DamageType);
...
defaultproperties
{
     ...
     BeamEffectClass(0)=Class'CSBadgerFix.ONSTurretBeamEffectGreen'
     BeamEffectClass(1)=Class'CSBadgerFix.ONSTurretBeamEffectGold'
     YawBone="Object01"
     PitchBone="Object02"
     PitchUpLimit=12500
     ...
     bInstantFire=True
     RedSkin=Texture'ONSToys1Tex.LinkTankTex.LinkTankLaserRed'
     BlueSkin=Texture'ONSToys1Tex.LinkTankTex.LinkTankLaserBlue'
     FireInterval=0.250000
     FireSoundClass=Sound'WeaponSounds.Misc.instagib_rifleshot'
     AmbientSoundScaling=1.300000
     FireForce="Laser01"
     DamageType=Class'CSBadgerFix.DamTypeLinkTankTurretBeams'
     DamageMin=12
     DamageMax=20
