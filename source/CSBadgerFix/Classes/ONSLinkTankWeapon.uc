...
// Link Tank weapon.
// ============================================================================
class ONSLinkTankWeapon extends ONSWeapon;
...
function Projectile SpawnProjectile(class<Projectile> ProjClass, bool bAltFire)
{
	...
	if (ONSLinkTank(Owner) != None)
		NumLinks = ONSLinkTank(Owner).GetLinks();
	else
		NumLinks = 0;
...
function SpawnBeamEffect(Vector Start, Rotator Dir, Vector HitLocation, Vector HitNormal, int ReflectNum)
{
	...
	if (ONSLinkTank(Owner) != None)
		FoundBeam = ONSLinkTank(Owner).Beam;
	...
	if (FoundBeam == None)
	{
		...
		if (ONSLinkTank(Owner) != None)
			ONSLinkTank(Owner).Beam = FoundBeam;
	}
...
//	log(self@"ceasefire"@bWasAltFire,'KDebug');
	if (bWasAltFire && Beam != None)
	{
		...
		if (ONSLinkTank(Owner) != None)
		{
			ONSLinkTank(Owner).Beam = None;
			ONSLinkTank(Owner).bBeaming = false;
		}
		//AmbientSound = None;
		...
		if (ONSLinkTank(Owner) != None)
		{
			...
			ONSLinkTank(Owner).bLinking = false;
		}
	}
}
...
function SetLinkTo(Pawn Other, optional bool bHealing)
{
	...
	if (LockedPawn != Other)
	{
	    if (LockedPawn != None && ONSLinkTank(Owner) != None)
	    {
			//log(self@"setlinkto"@other@"current"@LockedPawn,'KDebug');
	        RemoveLink(1 + ONSLinkTank(Owner).GetLinks(), Instigator);
...
defaultproperties
{
     ...
     HealMult=1.500000
     BeamEffectClass=Class'CSBadgerFix.ONSLinkTankBeamEffect'
     MakeLinkSound=Sound'WeaponSounds.LinkGun.LinkActivated'
     LinkBreakDelay=0.500000
     MomentumTransfer=2000.000000
     AltDamageType=Class'CSBadgerFix.DamTypeLinkTankBeam'
     AltDamage=15
     ...
     ProjectileClass=Class'CSBadgerFix.ONSLinkTankProjectile'
