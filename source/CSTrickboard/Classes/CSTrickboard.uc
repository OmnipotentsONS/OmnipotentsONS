...
var     int      armRotRTemp;
var     int      bodyLocR;
var     int      bodyLocRTemp;
var     int      bodyVel;

var     int      superJumpCount;
...
simulated function SetDriverPositionReset() {
...
simulated function SetDriverPositionOriginal(Pawn Driver) {
...
simulated function ClientKDriverLeave(PlayerController pc)
{
    ...
    self.SetBoneRotation('root', rot(0, 0, 0), 1.0);
    SetDriverPositionOriginal(pc.Pawn); // resets the position to the original position only if he could leave
...
//           Driver.SetBoneRotation('Bip01', bodyRot, 1.0);
           ...
           legRot.Roll = -8000;
           legRot.Pitch = 0;
//           Driver.SetBoneRotation('Bip01 L Calf', legRot, 1.0);
...
/*
    ...
    */
	...
	for (i = 0; i < Weapons.Length; i++)
		if (CSTrickboardWeapon(Weapons[i]) != None)
			CSTrickboardWeapon(Weapons[i]).UpdateLinkColor(Color);
}
...
// ============================================================================
// DrawHud
// Update HUD with that little link circle thingy
// ============================================================================
function DrawHUD(Canvas Canvas)
{
	if (Level.NetMode != NM_DedicatedServer) {
		if (OurHud == None) {
			foreach DynamicActors(class'HudCTeamDeathMatch',OurHud)
				break;
		}

        if (OurHud != None)
        	OurHud.Links = GetLinks();
...
simulated event KApplyGrappleForce(out vector Force, out vector Torque)
{
	...
	if (boardweapon != none && boardweapon.LockedPawn != none)
	{
        ...
        //PawnDir = Normal(LockedPawn.Velocity);
...
defaultproperties
{
    bCanFly=true
     ...
     DuckSound=Sound'CSTrickboard.spinattack'
