...
class Weapon_Phoenix extends Weapon
    config(user)
    HideDropDown
	CacheExempt;
...
{
    local Proj_Phoenix Phoenix;

        Phoenix = Spawn(class'Proj_Phoenix',,, Start, Dir);
        Excalibur(Instigator).Controller.Adrenaline = 0.0;
        Excalibur(Instigator).DeleteInventory(self);
        Excalibur(Instigator).DiscardPhoenix();
...
defaultproperties
{
     FireModeClass(0)=Class'CSAPVerIV.WeaponFire_PhoenixFire'
