...
//=============================================================================
class FighterTeleporter extends SmallNavigationPoint
	placeable;
...
simulated function bool Accept( actor Incoming, Actor Source )
{
	...
	if ( AirPower_Fighter(Incoming) != None )
	{
		...
		if ( !AirPower_Fighter(Incoming).SetLocation(Location) )
		{
		...
		{
			newRot.Roll = 0;
			AirPower_Fighter(Incoming).SetRotation(newRot);
			AirPower_Fighter(Incoming).SetViewRotation(newRot);
			AirPower_Fighter(Incoming).ClientSetRotation(newRot);

            LastFired = Level.TimeSeconds;
		}
		if ( AirPower_Fighter(Incoming).Controller != None )
		{
			AirPower_Fighter(Incoming).Controller.MoveTimer = -1.0;
			AirPower_Fighter(Incoming).Anchor = self;
			AirPower_Fighter(Incoming).SetMoveTarget(self);
		}
...
{
	local FighterTeleporter D,Dest[16];
	local int i;

	if( (InStr( URL, "/" ) >= 0) || (InStr( URL, "#" ) >= 0) )
	{
		// Teleport to a level on the net.
		if( (Role == ROLE_Authority) && (AirPower_Fighter(Other) != None)
			&& AirPower_Fighter(Other).IsHumanControlled() )
			Level.Game.SendPlayer(PlayerController(AirPower_Fighter(Other).Controller), URL);
	}
	else
	{
		// Teleport to a random teleporter in this local level, if more than one pick random.

		foreach AllActors( class 'FighterTeleporter', D )
...
{
	local vector Dist2D;
	if ( bEnabled && (FighterTeleporter(Other.Controller.RouteCache[1]) != None)
