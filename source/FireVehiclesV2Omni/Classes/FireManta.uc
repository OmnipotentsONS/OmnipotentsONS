//-----------------------------------------------------------
// Cherufe Fire Manta by ~Ca5i3l~ (c) 2016.
//-----------------------------------------------------------
class FireManta extends ONSHoverCraft;
...
#exec OBJ LOAD FILE=..\textures\CherufeV2Omni.utx
...
simulated function CheckJumpDuck()
{
    ...
    else if (DuckCountdown <= 0.0 && (Rise < 0 || bWeaponIsAltFiring))
    {
        if (!bHoldingDuck)
        {
			...
			if(Level.NetMode != NM_DedicatedServer)
			{
				DuckEffect = Spawn(class'FireVehiclesV2Omni.FlameDuckEmitter');
...
defaultproperties
{
     ...
     JumpSound=Sound'WeaponSounds.Misc.ballgun_change'
     DuckSound=Sound'WeaponSounds.Misc.ballgun_launch'
     ...
     DriverWeapons(0)=(WeaponClass=Class'FireVehiclesV2Omni.FireMantaGun',WeaponBone="PlasmaGunAttachment")
     bHasAltFire=False
     RedSkin=Texture'CherufeV2Omni.textures.firemantaRed'
     BlueSkin=Texture'CherufeV2Omni.textures.firemantaBlue'
     ...
     VehicleNameString="Cherufe 2.94"
     ...
     End Object
     KParams=KarmaParamsRBFull'FireVehiclesV2Omni.FireManta.KParams0'
