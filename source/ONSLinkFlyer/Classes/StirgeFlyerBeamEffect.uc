// ============================================================================
// StirgeFlyer                                                        ItsMeAgain
// Beam Effect, Mostly copied from LinkTank
// ============================================================================
class StirgeFlyerBeamEffect extends LinkBeamEffect
    notplaceable;
	...
	if ( Level.NetMode != NM_DedicatedServer )
	{
		...
		if ( Sparks == None && EffectIsRelevant(EndEffect, false) )
		{
			...
			if ( (P == Instigator.Controller) || CheckMaxEffectDistance(P, Location) )
				Sparks = Spawn(class'StirgeSparksPurple', self);
		}
	}
    ...
    if ( Links != OldLinks || LinkColor != OldLinkColor || MuzFlash != OldMuzFlash )
    {
		...
		if ( Level.NetMode != NM_DedicatedServer )
		{
			for (c=0; c<MAX_CHILDREN; c++)
			{
				...
				{
					if ( Child[c] == None )
						Child[c] = Spawn(class'StirgeBeamChild', self);
