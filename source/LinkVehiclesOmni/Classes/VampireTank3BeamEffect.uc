...
class VampireTank3BeamEffect extends LinkBeamEffect;

var VampireTank3Gun VampTankGun;
...
simulated function SetBeamLocation()
{
    ...
    {
		VampTankGun.CalcWeaponFire();
        StartEffect = VampTankGun.WeaponFireLocation;
...
simulated function Vector SetBeamRotation()
{
    if ( (Instigator != None) && PlayerController(Instigator.Controller) != None )
        SetRotation( VampTankGun.WeaponFireRotation);
    else
        SetRotation( VampTankGun.WeaponFireRotation );
    ...
    if ( Links != OldLinks || LinkColor != OldLinkColor || MuzFlash != OldMuzFlash )
    {
		...
		if ( Level.NetMode != NM_DedicatedServer )
		{
			for (c=0; c<MAX_CHILDREN; c++)
			{
				...
				{
					if ( Child[c] == None )
						Child[c] = Spawn(class'VampireTank3BeamChild', self);
