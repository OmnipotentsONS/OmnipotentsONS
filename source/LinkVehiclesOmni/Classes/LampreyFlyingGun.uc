class LampreyFlyingGun extends ONSWeapon;

#exec OBJ LOAD FILE=..\Animations\ONSWeapons-A.ukx

var bool bIsFiring;
var LampreyFlyingManta MyLamprey;
var()	float	InheritVelocityScale; // Amount of vehicles velocity

var LampreyFlyingBeamEffect			Beam, BeamLeft;  // two beams.
var class<LampreyFlyingBeamEffect>	BeamEffectClass;
...
simulated function tick(float dt)
{
	...
	//	local float ls;
		...
		local LampreyFlyingBeamEffect LB;
		...
		MyLamprey = LampreyFlyingManta(Owner);
	    ...
	    if ( Instigator.Role < ROLE_Authority ) {
				if ( Beam == None )
					ForEach DynamicActors(class'LampreyFlyingBeamEffect', LB )
...
defaultproperties
{
     BeamEffectClass=Class'LinkVehiclesOmni.LampreyFlyingBeamEffect'
