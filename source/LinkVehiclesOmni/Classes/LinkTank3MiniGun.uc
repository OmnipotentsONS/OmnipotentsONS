...
// Link Tank weapon.
// ============================================================================
class LinkTank3MiniGun extends ONSWeapon;
...
function Projectile SpawnProjectile(class<Projectile> ProjClass, bool bAltFire)
{
	...
	if (LinkTank3Mini(Owner) != None)
		NumLinks = LinkTank3Mini(Owner).GetLinks();
	else
		NumLinks = 0;
...
function SpawnBeamEffect(Vector Start, Rotator Dir, Vector HitLocation, Vector HitNormal, int ReflectNum)
{
	...
	if (LinkTank3Mini(Owner) != None)
		FoundBeam = LinkTank3Mini(Owner).Beam;
...
//	log(self@"ceasefire"@bWasAltFire,'KDebug');
	if (bWasAltFire && Beam != None)
	{
		...
		if (LinkTank3Mini(Owner) != None)
		{
			...
			LinkTank3Mini(Owner).bLinking = false;
		}
	}
}
...
simulated event Tick(float dt)
{
	...
	//log(Level.TimeSeconds@self@"TICK -- Owner"@Owner@"LinkTank of Owner"@LinkTank3Mini(Owner),'KDebug');
	if (LinkTank3Mini(Owner) != None)
	{
		LinkTank = LinkTank3Mini(Owner);
...
//		log("if role < role authority then do shit",'KDebug');
 ...
 if(LinkedVehicle.HealDamage(AdjustedDamage, Instigator.Controller, DamageType))
			...
			/* The math on the below is all fubar'd
			if(LinkedVehicle.HealDamage(AdjustedDamage, Instigator.Controller, DamageType))
	      {
	        score = 1;
...
function SetLinkTo(Pawn Other, optional bool bHealing)
{
	...
	if (LockedPawn != Other)
	{
	    if (LockedPawn != None && LinkTank3Mini(Owner) != None)
	    {
			//log(self@"setlinkto"@other@"current"@LockedPawn,'KDebug');
	        RemoveLink(1 + LinkTank3Mini(Owner).GetLinks(), Instigator);
	    ...
	    //if (LinkTank3Mini(Owner) != None)
	    //	LinkTank3Mini(Owner).bLinking = bIsHealingObjective;

	    if (LockedPawn != None)
	    {
			if (LinkTank3Mini(Owner) != None)
			{
		        if (!AddLink(1 + LinkTank3Mini(Owner).GetLinks(), Instigator))
		        {
		            bFeedbackDeath = true;
		        }
		        			//log(Level.TimeSeconds@Self@"Set Link Tank bLinking to TRUE in SetLinkTo"@Other,'KDebug');
		        LinkTank3Mini(Owner).bLinking = true;
			}
...
state ProjectileFireMode
{
	...
	{
		if (LinkTank3Mini(Owner).Beam != None) CeaseFire(C);
...
defaultproperties
{
     ...
     ProjectileClass=Class'LinkVehiclesOmni.LinkTank3MiniProjectile'
