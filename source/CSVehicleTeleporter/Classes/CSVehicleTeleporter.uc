...
//
// - Quick mod to make these work with mechs - snarf
//=============================================================================

class CSVehicleTeleporter extends Teleporter
	placeable;

#exec Texture Import File=Textures\S_VehicleTeleport.bmp Name=S_VehicleTeleport Mips=Off MASKED=1
...
function PostBeginPlay()
{
	...
	for ( i=0; i<PathList.Length; i++ )
		if ( CSVehicleTeleporter(PathList[i].End) != None )
		{
  ...
  local vector LocationOffset;
	...
	if ( Pawn(Incoming) != None )
	{
     ...
     // for CSMechs eg. CSShockMech etc, not dependent on class just class names
		...
		if ( !Pawn(Incoming).SetLocation(Location+LocationOffset) )
		{
	...
	{
		if ( !Incoming.SetLocation(Location+LocationOffset) )
		{
			Enable('Touch');
...
{
	local CSVehicleTeleporter D,Dest[16];
	local int i;
	...
	else
	{
		...
		foreach AllActors( class 'CSVehicleTeleporter', D )
...
function Actor SpecialHandling(Pawn Other)
{
	...
	if ( bEnabled && (CSVehicleTeleporter(Other.Controller.RouteCache[1]) != None)
...
function ClearDest(actor Incoming, Actor Source)
{
	...
	foreach CollidingActors(class'Pawn', BlockingPawn, CheckDistance,self.location) // is self.location rearly needed?
	{
		If ( BlockingPawn.IsA('Vehicle') )
		{
			...
			If ( Pawn(Incoming).Controller != None )
				Blockingpawn.Died(Pawn(Incoming).Controller, class'DamTypeCSVehicleTeleport', Location); // kill vehicle
			else
...
defaultproperties
{
     bKeepFlyingUpright=True
     ReachSpecCollHeight=2000
     ReachSpecCollRadius=2000
     bVehicleDestination=True
     Texture=Texture'S_VehicleTeleport'
}
