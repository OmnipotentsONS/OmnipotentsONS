...
//
//-----------------------------------------------------------
class MinotaurClassicRocketProjectile extends Projectile;
...
simulated function HurtRadius(float DamageAmount, float DamageRadius, class<DamageType> DamageType, float Momentum, vector HitLocation)
{
	...
	foreach VisibleCollidingActors(class 'Actor', Victims, DamageRadius, HitLocation)
	{
		...
		{
			ldir = Victims.Location - HitLocation;
			dist = FMax(1, VSize(ldir));
			ldir = ldir / dist;
			...
			Victims.TakeDamage(damageScale * DamageAmount, Instigator,
						Victims.Location - 0.5 * (Victims.CollisionHeight + Victims.CollisionRadius) * ldir,
						(damageScale * Momentum * ldir), DamageType);
...
defaultproperties
{
     ...
     ExplosionDecal=Class'MinotaurClassic.MinotaurClassicRocketScorch'
