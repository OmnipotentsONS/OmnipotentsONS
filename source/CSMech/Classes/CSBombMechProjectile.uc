...
class CSBombMechProjectile extends Projectile;
...
simulated function Destroyed()
{
    ...
    if ( !bNoFX )
    {
		...
		{
			Spawn(class'CSBombMechExplosionEffect',,, Location, rotator(vect(0,0,1)));
			Spawn(ExplosionDecal,self,, Location, rotator(vect(0,0,-1)));
		}
		PlaySound(sound'WeaponSounds.BExplosion3',,3.5*TransientSoundVolume,,1500,0.8);
	}
...
simulated function PostBeginPlay()
{
    ...
    if ( Level.NetMode != NM_DedicatedServer)
    {
		...
		if ( (PC.ViewTarget != None) && VSize(PC.ViewTarget.Location - Location) < 5500 )
	        Trail = Spawn(class'CSBombMechBombSmokeTrail', self,, Location, Rotation);
    }
...
simulated function PostNetBeginPlay()
{
	...
	{
		if (Team == 1)
			Beacon = spawn(class'CSBombMechBeaconBlue', self);
		else
			Beacon = spawn(class'CSBombMechBeaconRed', self);
