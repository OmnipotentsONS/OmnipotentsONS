class CSFlakMechFlakShell extends flakshell;
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
    ...
    local rotator rot;
    local int i;
    local CSFlakMechFlakChunk NewChunk;
	...
	if ( Role == ROLE_Authority )
	{
		...
		for (i=0; i<13; i++)
		{
			...
			NewChunk = Spawn( class 'CSFlakMechFlakChunk',, '', Start, rot);
		}
	}
    Destroy();
}
...
simulated function SpawnEffects( vector HitLocation, vector HitNormal )
{
	...
	if ( EffectIsRelevant(Location,false) )
	{
		...
		if ( (PC.ViewTarget != None) && VSize(PC.ViewTarget.Location - Location) < 3000 )
			spawn(class'CSFlakMechFlakExplosion',,,HitLocation + HitNormal*16 );
		spawn(class'CSFlakMechFlashExplosion',,,HitLocation + HitNormal*16 );
		spawn(class'CSFlakMechSmokeRing',,,HitLocation + HitNormal*16, rotator(HitNormal) );
...
defaultproperties
{
    ExplosionDecal=class'CSFlakMechFlakScorch'
	TossZ=+225.0
	bProjTarget=True
    //speed=1200.000000
    //Damage=90.000000
    ...
    MyDamageType=class'CSFlakMechDamTypeFlakShell'
    LifeSpan=6.000000
