class CSBioBomberBioGlob extends BioGlob;


simulated function PostBeginPlay()
{
    if(CSBioBomberBioGlob(owner) != None)
    {
        GoopLevel=Clamp(CSBioBomberBioGlob(Owner).GoopLevel-1, 0, MaxGoopLevel);
...
simulated function Destroyed()
{
    ...
    {
        Spawn(class'CSBioBomberBigGoopSmoke');
        Spawn(class'CSBioBomberBigGoopSparks');
    }
	if ( Fear != None )
		Fear.Destroy();
    if (Trail != None)
...
auto state Flying
{
    simulated function Landed( Vector HitNormal )
    {
        ...
        */
		...
		spawn(class'CSBioBomberBombDecal',,,, rotator(-HitNormal));
